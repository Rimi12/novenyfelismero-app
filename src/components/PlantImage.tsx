...
const PlantImage: React.FC<PlantImageProps> = ({ latinName, hungarianName }) => {
  ...
  useEffect(() => {
    const fetchWikiImage = async () => {
      ...
      try {
        // Wikipedia API hívás a latin név alapján
        ...
        if (thumbnail && thumbnail.source) {
          ...
          // Ha nincs Wikipedia kép, fallback a loremflickr-re egy egyedi sorszámmal (seed)
          const keywords = `${latinName.replace(/ /g, ',')},plant,flower`;
          const fallbackUrl = `https://loremflickr.com/800/600/${keywords}?lock=${latinName.length}`;
          setImageUrl(fallbackUrl);
        }
      } catch (err) {
        console.error("Wiki API error:", err);
        // Hiba esetén is fallback
        const keywords = `${latinName.replace(/ /g, ',')},plant,flower`;
        setImageUrl(`https://loremflickr.com/800/600/${keywords}?lock=${latinName.length}`);
      }
    };
  ...
  return (
    <div className="relative w-full aspect-video rounded-2xl overflow-hidden bg-green-100 shadow-inner flex items-center justify-center">
      {loading && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-green-50 z-10 transition-opacity duration-300">
          <div className="w-12 h-12 border-4 border-green-200 border-t-green-600 rounded-full animate-spin"></div>
          <p className="mt-4 text-green-700 font-medium animate-pulse">Kép keresése...</p>
        </div>
      )}
      ...
      {error && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-100 text-gray-500 p-4 text-center">
          ...
          <p>Kép nem elérhető: {hungarianName}</p>
        </div>
      )}
    </div>
  );
